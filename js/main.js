var currentstate, loopGameloop, loopPipeloop, removePipeloop, debugmode = !1,
...
function startGame() {
    currentstate = states.GameScreen, $("#splash").stop(), $("#splash").transition({
        opacity: 0
    }, 300, "ease"), setBigScore(), debugmode && $(".boundingbox").show(), loopGameloop = setInterval(gameloop, 1e3 / 60), loopPipeloop = setInterval(updatePipes, 1400), removePipeloop = setInterval(removePipes, 0), playerJump()
}
...
function playerDead() {
    ...
    $("#player").transition({
        ...
        rotate: 90
    }, 1e3, "easeInOutCubic"), currentstate = states.ScoreScreen, clearInterval(loopGameloop), clearInterval(loopPipeloop), clearInterval(removePipeloop), loopGameloop = null, loopPipeloop = null, removePipeloop = null, isIncompatible.any() ? showScore() : soundHit.play().bindOnce("ended", function () {
...
function soundSet() {
    ...
    buzz.all().setVolume(30), "false" == e ? buzz.all().setVolume(30) : buzz.all().setVolume(0)
}
muteSound == null && localStorage.setItem("mutesound", !1), $(window).resize(function () {
    var e = $(window).width();
